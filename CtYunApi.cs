...
namespace CtYun
{
    internal class CtYunApi
    {
        ...
        // 日志回调
        public Action<string, ConsoleColor> OnLog { get; set; }
        ...
        public async Task<bool> LoginAsync(string userphone, string password)
        {
            for (int i = 1; i < 4; i++)
            {
                ...
                OnLog?.Invoke($"重试{i}, Login Error:{result.Msg}", ConsoleColor.Red);
        ...
        public async Task<bool> GetSmsCodeAsync(string userphone)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                if (!string.IsNullOrEmpty(captchaCode))
                {
                    ...
                    OnLog?.Invoke($"重试{i}, GetSmsCode Error:{result.Msg}", ConsoleColor.Red);
                }
            }
            return false;
        }
        ...
        public async Task<bool> BindingDeviceAsync(string verificationCode)
        {
            ...
            OnLog?.Invoke("BindingDevice Error:" + result.Msg, ConsoleColor.Red);
            return false;
        }
        ...
        private async Task<ChallengeData> GetGenChallengeDataAsync()
        {
            ...
            OnLog?.Invoke("GetGenChallengeDataAsync Error:" + result.Msg, ConsoleColor.Red);
            return null;
        }
        ...
        private async Task<byte[]> GetLoginCaptcha(string userphone)
        {
            ...
            {
                OnLog?.Invoke("登录验证码获取错误：" + ex.Message, ConsoleColor.Red);
                return null;
            }
        }
        ...
        private async Task<byte[]> GetSmsCodeCaptcha()
        {
            ...
            {
                OnLog?.Invoke("短信验证码获取错误：" + ex.Message, ConsoleColor.Red);
                return null;
            }
        }
        ...
        private async Task<string> GetCaptcha(byte[] img)
        {
            try
            {
                OnLog?.Invoke("正在识别验证码.", ConsoleColor.White);
                ...
                OnLog?.Invoke("识别结果：" + result, ConsoleColor.Green);
                using var doc = JsonDocument.Parse(result);
                return doc.RootElement.GetProperty("data").GetString();
            }
            catch (Exception ex)
            {
                OnLog?.Invoke("验证码识别错误：" + ex.Message, ConsoleColor.Red);
                return "";
            }
        }
        ...
        public async Task<List<Desktop>> GetLlientListAsync()
        {
            ...
            {
                OnLog?.Invoke("获取设备信息错误。" + ex.Message, ConsoleColor.Red);
                return null;
            }
        }
